...
List<CandleData> _generateDummyCandles(int count) {
  ...
  for (int i = 0; i < count; i++) {
    ...
    final waveToday = 200.0 + (i * 0.3) + (math.sin(i / 8.0) * 18.0) + (math.sin(i / 3.0) * 5.0);
    final waveYesterday = i > 0
        ? 200.0 + ((i - 1) * 0.3) + (math.sin((i - 1) / 8.0) * 18.0) + (math.sin((i - 1) / 3.0) * 5.0)
        : 200.0;
    ...
    if (roll < 0.20) {
      change += 15.0 + random.nextDouble() * 15.0; // Big green candle (15 to 30 points)
    } else if (roll < 0.40) {
      change -= 15.0 + random.nextDouble() * 15.0; // Big red candle (15 to 30 points)
    }
    ...
    final high = math.max(open, close) + random.nextDouble() * (3.0 + bodySize * 0.2);
    final low = math.min(open, close) - random.nextDouble() * (3.0 + bodySize * 0.2);

    // Volume bars (highly volatile days have higher trading volume)
    final volume = 150.0 + random.nextDouble() * 500.0 + (bodySize > 12.0 ? 400.0 : 0.0);
